...
// Set debugType to 1 to show where skin color is taken from, and 2 to show the skin mask in a new
// window (for desktop).
void cartoonifyImage(
    Mat srcColor, Mat dst, bool sketchMode, bool alienMode, bool evilMode, int debugType)
{
    ...
    if (alienMode) {
        ...
        // Detects the color of the pixels in the middle of the image, then changes the color of
        // that region to green.
    ...
    // Clear the output image to black, so that the cartoon line drawings will be black (ie: not
    // drawn).
...
// Detects the color of the pixels in the middle of the image, then changes the color of that region
// to green.
void changeFacialSkinColor(Mat smallImgBGR, Mat bigEdges, int debugType)
{
    ...
    // Apply flood fill on many points around the face, to cover different shades & colors of the
    // face. Note that these values are dependent on the face outline, drawn in
    // drawFaceStickFigure().
    ...
    // Instead of drawing into the "yuv" image, just draw 1's into the "maskPlusBorder" image, so we
    // can apply it later. The "maskPlusBorder" is initialized with the edges, because floodFill()
    // will not go across non-zero mask pixels.
    ...
    // "edgeMask" just contains edges. So to get just the skin pixels, we can remove the edges from
    // it.
    mask -= edgeMask;
...
void removePepperNoise(Mat& mask)
{
    ...
    for (auto y = 2; y < mask.rows - 2; y++) {
        ...
        for (auto x = 2; x < mask.cols - 2; x++) {
            ...
            // If the current pixel is black, but all the pixels on the 2-pixel-radius-border are
            // white (ie: it is a small island of black pixels, surrounded by white), then delete
            // that island.
            if (v == 0) {
                bool allAbove = *(pUp2 - 2) && *(pUp2 - 1) && *(pUp2) && *(pUp2 + 1) && *(pUp2 + 2);
                bool allLeft = *(pUp1 - 2) && *(pThis - 2) && *(pDown1 - 2);
                bool allBelow
                    = *(pDown2 - 2) && *(pDown2 - 1) && *(pDown2) && *(pDown2 + 1) && *(pDown2 + 2);
...
void drawFaceStickFigure(Mat dst)
{
    ...
    int faceH = sh / 2 * 70
        / 100; // "faceH" is actually half the face height (ie: radius of the ellipse).
    ...
    ellipse(faceOutline, Point(sw / 2 - eyeX, sh / 2 - eyeY), Size(eyeW, eyeH), 0, 180 + eyeA,
        360 - eyeA, color, thickness, LINE_AA);
    // Draw the bottom of the right eye.
    ellipse(faceOutline, Point(sw / 2 - eyeX, sh / 2 - eyeY - eyeYshift), Size(eyeW, eyeH), 0,
        0 + eyeA, 180 - eyeA, color, thickness, LINE_AA);
    ...
    putText(faceOutline, "Put your face here", Point(sw * 23 / 100, sh * 10 / 100), fontFace,
        fontScale, color, fontThickness, LINE_AA);
