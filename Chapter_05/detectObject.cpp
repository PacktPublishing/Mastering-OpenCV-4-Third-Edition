...
// Search for objects such as faces in the image using the given parameters, storing the multiple
// cv::Rects into 'objects'. Can use Haar cascades or LBP cascades for Face Detection, or even eye,
// mouth, or car detection. Input is temporarily shrunk to 'scaledWidth' for much faster detection,
// since 200 is enough to find faces.
    ...
    int scaledWidth, int flags, Size minFeatureSize, float searchScaleFactor, int minNeighbors)
{
    ...
    cascade.detectMultiScale(
        equalizedImg, objects, searchScaleFactor, minNeighbors, flags, minFeatureSize);
...
// Search for just a single object in the image, such as the largest face, storing the result into
// 'largestObject'. Can use Haar cascades or LBP cascades for Face Detection, or even eye, mouth, or
// car detection. Input is temporarily shrunk to 'scaledWidth' for much faster detection, since 200
// is enough to find faces. Note: detectLargestObject() should be faster than detectManyObjects().
void detectLargestObject(
    const Mat& img, CascadeClassifier& cascade, Rect& largestObject, int scaledWidth)
{
    ...
    vector<Rect> objects;
    detectObjectsCustom(
        img, cascade, objects, scaledWidth, flags, minFeatureSize, searchScaleFactor, minNeighbors);
    if (objects.size() > 0) {
...
// Input is temporarily shrunk to 'scaledWidth' for much faster detection, since 200 is enough to
// find faces. Note: detectLargestObject() should be faster than detectManyObjects().
void detectManyObjects(
    const Mat& img, CascadeClassifier& cascade, vector<Rect>& objects, int scaledWidth)
{
