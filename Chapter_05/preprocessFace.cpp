...
const double DESIRED_LEFT_EYE_X
    = 0.16; // Controls how much of the face is visible after preprocessing.
...
// Keeps the center of the rectangle at the same place, rather than just dividing all values by
'scale'. Rect scaleRectFromCenter(const Rect wholeFaceRect, float scale)
{
    ...
    faceRect.x = cvRound(faceCenterX - newWidth * 0.5f);    // Move the region so that the center is
still the same spot. faceRect.y = cvRound(faceCenterY - newHeight * 0.5f);

    return faceRect;
}
*/

// Search for both eyes within the given face image. Returns the eye centers in 'leftEye' and
// 'rightEye', or sets them to (-1,-1) if each eye was not found. Note that you can pass a 2nd
// eyeCascade if you want to search eyes using 2 different cascades. For example, you could use a
// regular eye detector as well as an eyeglasses detector, or a left eye detector as well as a right
// eye detector. Or if you don't want a 2nd eye detection, just pass an uninitialized
// CascadeClassifier. Can also store the searched left & right eye regions if desired.
...
{
    // Skip the borders of the face, since it is usually just hair and ears, that we don't care
    // about.
    /*
        // For "2splits.xml": Finds both eyes in roughly 60% of detected faces, also detects closed
       eyes. const float EYE_SX = 0.12f; const float EYE_SY = 0.17f; const float EYE_SW = 0.37f;
    ...
    */
    ...
    // For default eye.xml or eyeglasses.xml: Finds both eyes in roughly 40% of detected faces, but
    // does not detect closed eyes.
    ...
    if (leftEyeRect.width > 0) { // Check if the eye was detected.
        leftEyeRect.x
            += leftX; // Adjust the left-eye rectangle because the face border was removed.
        leftEyeRect.y += topY;
        leftEye
            = Point(leftEyeRect.x + leftEyeRect.width / 2, leftEyeRect.y + leftEyeRect.height / 2);
    } else {
        leftEye = Point(-1, -1); // Return an invalid point
    }

    if (rightEyeRect.width > 0) { // Check if the eye was detected.
        rightEyeRect.x += rightX; // Adjust the right-eye rectangle, since it starts on the right
                                  // side of the image.
        rightEyeRect.y
            += topY; // Adjust the right-eye rectangle because the face border was removed.
        rightEye = Point(
            rightEyeRect.x + rightEyeRect.width / 2, rightEyeRect.y + rightEyeRect.height / 2);
    } else {
...
{
    // It is common that there is stronger light from one half of the face than the other. In that
    // case, if you simply did histogram equalization on the whole face then it would make one half
    // dark and one half bright. So we will do histogram equalization separately on each face half,
    // so they will both look similar on average. But this would cause a sharp edge in the middle of
    // the face, because the left half and right half would be suddenly different. So we also
    // histogram equalize the whole image, and in the middle part we blend the 3 images together for
    // a smooth brightness transition.
    ...
    // 3) Combine the left half and right half and whole face together, so that it has a smooth
    // transition.
    for (int y = 0; y < h; y++) {
        for (int x = 0; x < w; x++) {
            int v;
...
// "srcImg" should be a copy of the whole color camera frame, so that it can draw the eye positions
// onto. If 'doLeftAndRightSeparately' is true, it will process left & right sides seperately, so
// that if there is a strong light on one side but not the other, it will still look OK. Performs
// Face Preprocessing as a combination of:
//  - geometrical scaling, rotation and translation using Eye Detection,
//  - smoothing away image noise using a Bilateral Filter,
//  - standardize the brightness on both left and right sides of the face independently using
//  separated Histogram Equalization,
//  - removal of background and hair using an Elliptical Mask.
// Returns either a preprocessed face square image or NULL (ie: couldn't detect the face and 2
// eyes). If a face is found, it can store the rect coordinates into 'storeFaceRect' and
// 'storeLeftEye' & 'storeRightEye' if given, and eye search regions into 'searchedLeftEye' &
// 'searchedRightEye' if given.
Mat getPreprocessedFace(Mat& srcImg, int desiredFaceWidth, CascadeClassifier& faceCascade,
    ...
    Rect* storeFaceRect, Point* storeLeftEye, Point* storeRightEye, Rect* searchedLeftEye,
    Rect* searchedRightEye)
{
    // Use square faces.
    int desiredFaceHeight = desiredFaceWidth;

    // Mark the detected face region and eye search regions as invalid, in case they aren't
    // detected.
    if (storeFaceRect)
    ...
    if (faceRect.width > 0) {
        ...
        // If the input image is not grayscale, then convert the BGR or BGRA color image to
        // grayscale.
        Mat gray;
        ...
        // Search for the 2 eyes at the full resolution, since eye detection needs max resolution
        // possible!
        Point leftEye, rightEye;
        detectBothEyes(
            gray, eyeCascade1, eyeCascade2, leftEye, rightEye, searchedLeftEye, searchedRightEye);
        ...
        if (leftEye.x >= 0 && rightEye.x >= 0) {
            ...
            // line up perfectly with ideal eye positions. This makes sure that eyes will be
            // horizontal, and not too far left or right of the face, etc.
            ...
            // Note that we use 'w' for the height instead of 'h', because the input face has 1:1
            // aspect ratio.
            ...
            // Give the image a standard brightness and contrast, in case it was too dark or had low
            // contrast.
            ...
            // Use the "Bilateral Filter" to reduce pixel noise by smoothing the image, but keeping
            // the sharp edges in the face.
            ...
            Point faceCenter
                = Point(desiredFaceWidth / 2, cvRound(desiredFaceHeight * FACE_ELLIPSE_CY));
            Size size = Size(cvRound(desiredFaceWidth * FACE_ELLIPSE_W),
                cvRound(desiredFaceHeight * FACE_ELLIPSE_H));
            ellipse(mask, faceCenter, size, 0, 0, 360, Scalar(255), FILLED);
            // imshow("mask", mask);

            // Use the mask, to remove outside pixels.
            Mat dstImg = Mat(
                warped.size(), CV_8U, Scalar(128)); // Clear the output image to a default gray.
            /*
