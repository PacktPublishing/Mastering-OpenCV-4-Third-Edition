...
// These functions will print using the LOG() function, using the same format as printf(). If you
// want it to be printed using a different function (such as for Android logcat output), then define
// LOG as your output function, otherwise it will use printf() by default.
#ifndef LOG
#ifdef __ANDROID__
...
#define DECLARE_TIMING(s)                                                                          \
    int64 timeStart_##s;                                                                           \
    int64 timeDiff_##s;                                                                            \
    int64 timeTally_##s = 0;                                                                       \
    int64 countTally_##s = 0;                                                                      \
    double timeMin_##s = DBL_MAX;                                                                  \
    double timeMax_##s = 0;                                                                        \
    int64 timeEnd_##s;
#define START_TIMING(s) timeStart_##s = cvGetTickCount()
#define STOP_TIMING(s)                                                                             \
    do {                                                                                           \
        timeEnd_##s = cvGetTickCount();                                                            \
        timeDiff_##s = (timeEnd_##s - timeStart_##s);                                              \
        timeTally_##s += timeDiff_##s;                                                             \
        countTally_##s++;                                                                          \
        timeMin_##s = MIN(timeMin_##s, timeDiff_##s);                                              \
        timeMax_##s = MAX(timeMax_##s, timeDiff_##s);                                              \
    } while (0)
...
#define GET_AVERAGE_TIMING(s)                                                                      \
    (double)(countTally_##s                                                                        \
            ? 0.001 * ((double)timeTally_##s / ((double)countTally_##s * cvGetTickFrequency()))    \
            : 0)
#define GET_TOTAL_TIMING(s)                                                                        \
    (double)(0.001 * ((double)timeTally_##s / ((double)cvGetTickFrequency())))
...
#define SHOW_TIMING(s, msg)                                                                        \
    LOG("%s time:\t %dms\t (ave=%dms min=%dms max=%dms, across %d runs).", msg,                    \
        cvRound(GET_TIMING(s)), cvRound(GET_AVERAGE_TIMING(s)), cvRound(GET_MIN_TIMING(s)),        \
        cvRound(GET_MAX_TIMING(s)), GET_TIMING_COUNT(s))
#define SHOW_TOTAL_TIMING(s, msg)                                                                  \
    LOG("%s total:\t %dms\t (ave=%dms min=%dms max=%dms, across %d runs).", msg,                   \
        cvRound(GET_TOTAL_TIMING(s)), cvRound(GET_AVERAGE_TIMING(s)), cvRound(GET_MIN_TIMING(s)),  \
        cvRound(GET_MAX_TIMING(s)), GET_TIMING_COUNT(s))
#define AVERAGE_TIMING(s) SHOW_TIMING(s, #s)
#define TOTAL_TIMING(s)                                                                            \
    do {                                                                                           \
        SHOW_TOTAL_TIMING(s, #s);                                                                  \
        CLEAR_AVERAGE_TIMING(s);                                                                   \
    } while (0)
...
// Remember to free the newly created image if imageDst is not given.
IplImage* drawIntGraph(const int* arraySrc, int nArrayLength, IplImage* imageDst DEFAULT(0),
...
void showFloatGraph(const char* name, const float* arraySrc, int nArrayLength,
    int delay_ms DEFAULT(500), IplImage* background DEFAULT(0));
...
// Create a YIQ image from the RGB image using an approximation of NTSC conversion(ref: "YIQ"
// Wikipedia page). Remember to free the generated YIQ image.
...
CvRect scaleRect(
    const CvRect rectIn, float scaleX, float scaleY, int w DEFAULT(0), int h DEFAULT(0));
...
// (height-1).
CvRect scaleRectInPlace(const CvRect rectIn, float scaleX, float scaleY,
    float borderX DEFAULT(0.0f), float borderY DEFAULT(0.0f), int w DEFAULT(0), int h DEFAULT(0));
...
void drawRectFilled(
    IplImage* img, const CvRect rect, const CvScalar color DEFAULT(CV_RGB(220, 0, 0)));
...
// desired, by cropping the desired region. Remember to free the new image later.
...
CvPoint2D32f mapRotatedImagePoint(const CvPoint2D32f pointOrig, const IplImage* image,
    float angleRadians, float scale DEFAULT(1.0f));
...
// 'combineImagesResized()' will resize all images to 300x300, whereas 'combineImages()' doesn't
// resize the images at all.
...
IplImage* blendImage(
    const IplImage* image1, const IplImage* image2, const IplImage* imageAlphaMask);
...
// Print the pixel values of the IplImage, to LOG() for easy debugging. maxElements of 0 will print
// all the data.
void printImagePixels(
    const IplImage* image, const char* label DEFAULT(0), int maxElements DEFAULT(300));
...
// Print the label and then contents of the Matrix (using "LOG()") for easy debugging. maxElements
// of 0 will print all the data.
