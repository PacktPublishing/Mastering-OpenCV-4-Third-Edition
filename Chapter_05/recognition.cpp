...
// Train the face recognition system on a given dataset, and recognize the person from a given
// image.
//////////////////////////////////////////////////////////////////////////////////////
...
// OpenCV 2.4)
Ptr<BasicFaceRecognizer> learnCollectedFaces(const vector<Mat> preprocessedFaces,
    const vector<int> faceLabels, const string facerecAlgorithm)
{
    Ptr<BasicFaceRecognizer> model;

    cout << "Learning the collected faces using the [" << facerecAlgorithm << "] algorithm ..."
         << endl;

    /*// Make sure the "contrib" module is dynamically loaded at runtime.
    // Requires OpenCV v2.4.1 or later (from June 2012), otherwise the FaceRecognizer will not
    compile or run! bool haveContribModule = initModule_contrib(); if (!haveContribModule) { cerr <<
    ...
    */
    ...
    if (model.empty()) {
        cerr << "ERROR: The FaceRecognizer algorithm [" << facerecAlgorithm
             << "] is not available in your version of OpenCV. Please make sure the 'contrib' "
                "module is built into your OpenCV library."
             << endl;
        exit(1);
    }

    // Do the actual training from the collected faces. Might take several seconds or minutes
    // depending on input!
...
// Convert the matrix row or column (float matrix) to a rectangular 8-bit image that can be
// displayed or saved. Scales the values to be between 0 to 255.
...
void showTrainingDebugData(
    const Ptr<BasicFaceRecognizer> model, const int faceWidth, const int faceHeight)
{
    try { // Surround the OpenCV calls by a try/catch block so we don't crash if some model
        ...
        for (int i = 0; i < min(20, eigenvectors.cols); i++) {
            ...
            // Note that clone() ensures it will be continuous, so we can treat it like an array,
            // otherwise we can't reshape it to a rectangle. Note that the FaceRecognizer class
            // already gives us L2 normalized eigenvectors, so we don't have to normalize them
            // ourselves.
...
// Generate an approximately reconstructed face by back-projecting the eigenvectors & eigenvalues of
// the given (preprocessed) face.
Mat reconstructFace(const Ptr<BasicFaceRecognizer> model, const Mat preprocessedFace)
{
    // Since we can only reconstruct the face for some types of FaceRecognizer models (ie:
    // Eigenfaces or Fisherfaces), we should surround the OpenCV calls by a try/catch block so we
    ...
    try {
        ...
        Mat projection
            = LDA::subspaceProject(eigenvectors, averageFaceRow, preprocessedFace.reshape(1, 1));
